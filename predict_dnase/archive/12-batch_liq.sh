...
for file in $DATA_DIR/$FILENAME; do
	...
	( 
		...
		10-bed2mat_liq.sh -i $REF_IDX -r $REF_FA -m $PWM_FILE -d $PWM_IDS $file $OUT_DIR

		if [ "$?" != "0" ]; then
			echo -e "Error when running 10-bed2mat_liq.sh on $file."
			exit -1
		fi 
		...
		if [ "$?" != "0" ]; then
			echo -e "Error when running 11-classifier.py (rf) on $file."
			exit -1
		fi 
